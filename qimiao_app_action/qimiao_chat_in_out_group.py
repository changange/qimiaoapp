...
class ChatGroup:
    ...
    def into_chat_detail(self):
        ...
        for i in range(5):
            ...
            if self.d(resourceId='com.qmnl.qmpd:id/title', instance=num_list).get_text() != '开播提醒':
                print(f'进入第：{num_list} 个聊天列表')
                self.d(resourceId='com.qmnl.qmpd:id/title', instance=num_list).click()     ##  num_list
                if self.s(resourceId='com.qmnl.qmpd:id/report_iv').wait(10):
                    print('已经成功进入 消息详情页')
                    return True
                elif self.s(resourceId='com.qmnl.qmpd:id/chat_group_title').wait(10):
                    print('已经成功进入 消息详情页')
                    return True
                else:
                    print('进入详细详情页失败~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        ...
        if self.d(text='发现小组').wait(3):
            ...
            if self.d(text='小组').wait(3):
                ...
                try:
                    ...
                    print(f'XXXXXXXXXXXX共：{join_list}个加入')
                except Exception:
